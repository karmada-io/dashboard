...
// sharedInformerFactory returns the shared informer factory, panicking if not initialized.
func sharedInformerFactory() karmadainformers.SharedInformerFactory {
	if factory == nil {
		panic("informer.sharedInformerFactory: factory is nil, call informer.Init() first")
	}
	return factory
}
...
func ResourceBindingIndexer() cache.Indexer {
	return sharedInformerFactory().Work().V1alpha2().ResourceBindings().Informer().GetIndexer()
}

// WorkIndexer returns the Work indexer.
func WorkIndexer() cache.Indexer {
	return sharedInformerFactory().Work().V1alpha1().Works().Informer().GetIndexer()
}
