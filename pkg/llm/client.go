...
func GetLLMClient() (*openai.Client, error) {
	...
	if globalLLMConfig.LLMAPIKey == "" {
		return nil, fmt.Errorf("%w: set LLM_API_KEY environment variable or configuration", ErrLLMAPIKeyNotConfigured)
	}
	...
	if globalLLMConfig.LLMEndpoint != "" {
		...
		if _, err := url.Parse(globalLLMConfig.LLMEndpoint); err != nil {
			return nil, fmt.Errorf("%w: %q is not a valid URL: %v",
				ErrInvalidEndpoint, globalLLMConfig.LLMEndpoint, err)
		}
