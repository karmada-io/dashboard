...
|----------------------------------------| --- | --- | --- | --- |
| OIDC Integration | @warjiang | @RainbowMango | @RainbowMango | 2026-04-12 |


## Summary

Karmada Dashboard currently relies on a Bearer-token login model in which users manually generate and paste JWT credentials (typically Kubernetes ServiceAccount tokens). This model introduces operational friction and does not align with enterprise authentication expectations.

For OIDC-based enterprise sign-in, representative implementation options include **[Dex](https://dexidp.io/docs/)**, **[Keycloak](https://www.keycloak.org/documentation)**, **[authentik](https://docs.goauthentik.io/)**, and **[ZITADEL](https://zitadel.com/docs)**. Based on the current scope and constraints, Dex is selected as the preferred option.

This selection is grounded in the following considerations: Dex provides direct upstream identity integration through connectors (LDAP/AD/SAML/OIDC), fully covers the required Dashboard OIDC login path, aligns well with the existing Kubernetes deployment model, and keeps backend implementation scope controlled (redirect handling, state validation, code exchange, and token verification).

In addition, adopting Dex preserves architectural flexibility. By maintaining the Dashboard as a standards-based OIDC client, future migration to broader IAM platforms remains feasible if organizational requirements evolve.

Accordingly, this proposal integrates **Dex** as the OIDC provider for Karmada Dashboard, enabling enterprise users to authenticate with existing organizational identities through the standard OAuth 2.0 authorization code flow.
