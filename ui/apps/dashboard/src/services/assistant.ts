...
export const getAssistantStream = (
  ...
  void fetchEventSource('/api/v1/assistant', {
    method: 'POST',
    headers: {
    ...
    onmessage(ev: { data: string }) {
      ...
      try {
        const response: StreamResponse = JSON.parse(ev.data) as StreamResponse;
...
export const getChatStream = (
  ...
  console.log('Sending message to chat with MCP:', {
    message,
    enableMCP,
  ...
  void fetchEventSource('/api/v1/chat', {
    ...
    onmessage(ev: { data: string }) {
      ...
      try {
        const response: ChatResponse = JSON.parse(ev.data) as ChatResponse;
