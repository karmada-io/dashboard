...
import {ClusterDetail, CreateCluster, UpdateCluster} from "@/services/cluster";
...
const NewClusterModal: FC<NewClusterModalProps> = (props) => {
    ...
    const [showAdvancedConfig, setShowAdvancedConfig] = useState(mode !== 'create')
    ...
    useEffect(() => {
        if (mode === 'edit') {
            ...
            if (clusterDetail?.objectMeta?.labels) {
                initData["labels"] = Object.keys(clusterDetail?.objectMeta?.labels).map(k => {
                    return {
                        key: k,
        ...
        >
            ...
            >
                <Form.Item
                    ...
                    {...formItemLayout}
                >
                    <Input placeholder={'请输入集群名称'} disabled={mode !== 'create'} className={'w-[200px]'}/>
                </Form.Item>
                <Form.Item
                    label="接入模式"
                    name="mode"
                        ...
                        {(fields, {add, remove}) => (
                            <>
                                {fields.map(({key, name}, index) =>
                                    ...
                                    >
                                        <div className={'flex flex-row items-center space-x-4'}>
                                            <Form.Item
                                                name={[name, 'key']}
                                                rules={[{required: true, message: 'label key必填'}]}
                                                noStyle
                                            >
                                                <Input placeholder="请输出label的key" className={'w-[150px]'}/>
                                            </Form.Item>
                                            <Form.Item
                                                name={[name, 'value']}
                                                rules={[{required: true, message: 'label value必填'}]}
                                                noStyle
                                            >
                                                <Input placeholder="请输出label的value" className={'w-[150px]'}/>
                                            </Form.Item>
                                            <Button
                                                onClick={() => remove(name)}
                                ...
                                <Form.Item {...formItemLayoutWithOutLabel}>
                                    <Button
                                        type="dashed" onClick={() => add()} block icon={<Icons.add/>}
                                        ...
                                        >
                                            <div className={'flex flex-row items-center space-x-4'}>
                                                <Form.Item
                                                    ...
                                                    noStyle
                                                >
                                                    <Input placeholder="请输出taint的key" className={'w-[200px]'}/>
                                                </Form.Item>
                                                {/*
                                                <Form.Item
                                                ...
                                                */}
                                                <Form.Item
                                                    ...
                                                    noStyle
                                                >
                                                    <Input placeholder="请输出taint的value"
                                                           className={'w-[200px]'}/>
                                                </Form.Item>
                                                <Form.Item
