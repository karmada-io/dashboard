      ...
      try {
        ...
        if (response.enabled) {
          void message.success(
            `MCP enabled, found ${response.tools.length} available tools`,
          );
        } else {
          void message.info('MCP not enabled or not available');
        }
      } catch (error) {
        console.error('Failed to load MCP tools:', error);
        void message.error('Failed to load MCP tools');
      }
    };
    ...
    if (enableMCP && mcpEnabled) {
      const controller = getChatStream(
        ...
        (error) => {
          ...
          void message.error('Chat request failed');
        },
        () => {
      ...
      const controller = getAssistantStream(
        ...
        (error) => {
          ...
          void message.error('Request failed');
        },
        () => {
          botMessageRef.current = null;
          currentControllerRef.current = null;
    ...
    <div style={{ padding: '20px', maxWidth: '1000px', margin: 'auto' }}>
        ...
        >
          <div>
            ...
            <p style={{ margin: '4px 0 0 0', color: '#666' }}>
              {mcpEnabled
                ? `MCP enabled, ${availableTools.length} tools available`
                : 'MCP not enabled'}
            </p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            {mcpEnabled && (
              <Tooltip
                title={`Available tools: ${availableTools.map((t) => t.name).join(', ')}`}
              >
              ...
              description={
                <div>
                  ...
                  {item.toolCalls &&
                    ...
                    renderToolCalls(item.toolCalls)}
                </div>
              }
            />
          </List.Item>
      ...
      <div style={{ display: 'flex', marginTop: '20px', gap: '10px' }}>
        <Input
          ...
          placeholder={
            enableMCP
              ? 'Enter your question, MCP will help query cluster information...'
              : 'Enter your question...'
          }
          style={{ flex: 1 }}
        />
