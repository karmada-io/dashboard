...
import {extractPropagationPolicy, GetIngress, Ingress} from "@/services/service.ts";
...
const IngressTable: FC<ServiceTableProps> = (props) => {
    const {
        ...
        onViewIngressContent, onDeleteIngressContent} = props
    ...
    return (
        <Table
            rowKey={(r: Ingress) => `${r.objectMeta.namespace}-${r.objectMeta.name}` || ''}
            columns={columns}
            loading={isLoading}
