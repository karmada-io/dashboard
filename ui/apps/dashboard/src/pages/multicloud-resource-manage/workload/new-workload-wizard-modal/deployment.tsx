...
const DeploymentForm: FC<DeploymentFormProps> = (props) => {
  ...
  const generateYaml = (values: any, template: any) => {
    ...
    if (values.updateStrategy) {
      _.set(yamlObj, 'spec.strategy.type', values.updateStrategy);
      
      if (values.updateStrategy === 'RollingUpdate') {
        _.set(yamlObj, 'spec.strategy.rollingUpdate.maxSurge', values.maxSurge);
        _.set(yamlObj, 'spec.strategy.rollingUpdate.maxUnavailable', values.maxUnavailable);
      }
    }
