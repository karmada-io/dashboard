...
import {
    ...
    deleteOverridePolicyResourceTest
} from './test-utils';
...
test('should delete clusteroverridepolicy successfully', async ({ page }) => {
    const testClusterOverridePolicyYaml = generateTestClusterOverridePolicyYaml();

    await deleteOverridePolicyResourceTest(page, {
        resourceType: 'clusteroverridepolicy',
        tabName: 'Cluster level',
        apiEndpointPattern: '/overridepolicy',  // Both namespace and cluster level use the same delete endpoint
        ...
        screenshotName: 'debug-clusteroverridepolicy-delete-kubectl.png'
    });
});
