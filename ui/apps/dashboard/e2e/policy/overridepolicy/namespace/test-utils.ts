...
/**
 * Creates a Kubernetes OverridePolicy using the Kubernetes JavaScript client.
...
export async function createK8sOverridePolicy(yamlContent: string): Promise<void> {
    try {
        ...
        throw new Error(`Failed to create overridepolicy: ${error.message}`);
    }
}

/**
 * Deletes a Kubernetes OverridePolicy using the Kubernetes JavaScript client.
...
export async function deleteK8sOverridePolicy(overridePolicyName: string, namespace: string = 'default'): Promise<void> {
    try {
        ...
        // Assert parameters are valid for test overridepolicy
        ...
        if (error.response?.status === 404 || error.statusCode === 404)  {
            // OverridePolicy not found - already deleted, this is fine
            return;
        }
        throw new Error(`Failed to delete overridepolicy: ${error.message}`);
    }
}

/**
 * Gets overridepolicy name from YAML content using proper YAML parsing.
...
export function getOverridePolicyNameFromYaml(yamlContent: string): string {
    ...
    if (!overridePolicyName) {
        throw new Error('Could not extract overridepolicy name from YAML');
    }
