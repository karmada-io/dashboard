...
test('should display overridepolicy namespace list', async ({ page }) => {
    // Open Policies menu
    ...
    // Verify Override Policy list table is visible
    const table = page.locator('table');
    await expect(table).toBeVisible({ timeout: 30000 });

    // Debug
    if (process.env.DEBUG === 'true') {
        await page.screenshot({ path: 'debug-overridepolicy-namespace-list.png', fullPage: true });
    }
});
