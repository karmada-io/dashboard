...
/**
 * Creates a Kubernetes ClusterPropagationPolicy using the Kubernetes JavaScript client.
...
export async function createK8sClusterPropagationPolicy(yamlContent: string): Promise<void> {
    try {
        ...
        // ClusterPropagationPolicy is cluster-scoped, so no namespace needed
        ...
        throw new Error(`Failed to create clusterpropagationpolicy: ${error.message}`);
    }
}

/**
 * Deletes a Kubernetes ClusterPropagationPolicy using the Kubernetes JavaScript client.
 ...
 */
export async function deleteK8sClusterPropagationPolicy(clusterPropagationPolicyName: string): Promise<void> {
    try {
        const k8sApi = createKarmadaApiClient();

        // Assert parameters are valid for test clusterpropagationpolicy
        expect(clusterPropagationPolicyName).toBeTruthy();
        expect(clusterPropagationPolicyName).not.toBe('');
        ...
        if (error.response?.status === 404 || error.statusCode === 404)  {
            // ClusterPropagationPolicy not found - already deleted, this is fine
            return;
        }
        throw new Error(`Failed to delete clusterpropagationpolicy: ${error.message}`);
    }
}

/**
 * Gets clusterpropagationpolicy name from YAML content using proper YAML parsing.
...
export function getClusterPropagationPolicyNameFromYaml(yamlContent: string): string {
    ...
    const clusterPropagationPolicyName = _.get(yamlObject, 'metadata.name');

    if (!clusterPropagationPolicyName) {
        throw new Error('Could not extract clusterpropagationpolicy name from YAML');
    }

    return clusterPropagationPolicyName;
}
