...
// PropagationPolicy Resource Configurations
...
/**
 * Generic function to create Karmada PropagationPolicy resources
 */
export async function createK8sResource<ResourceType extends keyof typeof RESOURCE_CONFIGS>(
    ...
    try {
        ...
        if (resourceType === 'propagationpolicy') {
        ...
        } else if (resourceType === 'clusterpropagationpolicy') {
...
/**
 * Generic function to delete Karmada PropagationPolicy resources
 */
...
// PropagationPolicy Resource Test Templates
...
/**
 * Generic test template for creating PropagationPolicy resources
 * This template maintains the exact same structure and flow as the original create tests
 */
export async function createPropagationPolicyResourceTest<T extends keyof typeof RESOURCE_CONFIGS>(
    page: Page,
...
/**
 * Generic test template for deleting PropagationPolicy resources
 * This template maintains the exact same structure and flow as the original delete tests
 */
export async function deletePropagationPolicyResourceTest<T extends keyof typeof RESOURCE_CONFIGS>(
    page: Page,
    ...
    // ========== DEBUG: START ==========
    console.log(`[DEBUG] Attempting to create resource: ${resourceName}`);
    console.log(`[DEBUG] Resource type: ${config.resourceType}`);
    // ========== DEBUG: END ==========
    ...
    try {
        ...
        // ========== DEBUG: START ==========
        console.log(`[DEBUG] Resource created successfully: ${resourceName}`);
        // ========== DEBUG: END ==========
    } catch (error) {
        // ========== DEBUG: START ==========
        console.error(`[DEBUG] Failed to create resource: ${resourceName}`, error);
        throw error;
        // ========== DEBUG: END ==========
    }
    ...
    try {
        await k8sApi.getNamespacedCustomObject({
            group: RESOURCE_CONFIGS[config.resourceType].apiGroup,
            version: RESOURCE_CONFIGS[config.resourceType].apiVersion,
            namespace: 'default',
            plural: RESOURCE_CONFIGS[config.resourceType].plural,
            name: resourceName
        });
        console.log(`[DEBUG] Resource verified in K8s API immediately after creation: ${resourceName}`);
        console.log(`[DEBUG] Resource can be queried from K8s successfully`);
    } catch (verifyError) {
        console.error(`[DEBUG] CRITICAL: Resource NOT found in K8s API immediately after creation: ${resourceName}`, verifyError);
    }
    // ========== DEBUG: END ==========
...
/**
 * Generic test template for displaying PropagationPolicy resource lists
 * This template maintains the exact same structure and flow as the original list tests
 */
export async function displayPropagationPolicyResourceListTest(
    page: Page,
...
/**
 * Generic test template for viewing PropagationPolicy resource details
 * This template maintains the exact same structure and flow as the original view tests
 */
export async function viewPropagationPolicyResourceTest<T extends keyof typeof RESOURCE_CONFIGS>(
    page: Page,
    ...
    // ========== DEBUG: START ==========
    console.log(`[DEBUG] [VIEW TEST] Attempting to create resource: ${resourceName}`);
    console.log(`[DEBUG] [VIEW TEST] Resource type: ${config.resourceType}`);
    // ========== DEBUG: END ==========
    ...
    try {
        await config.createResource(config.yamlContent);
        // ========== DEBUG: START ==========
        console.log(`[DEBUG] [VIEW TEST] Resource created successfully: ${resourceName}`);
        // ========== DEBUG: END ==========
    } catch (error) {
        // ========== DEBUG: START ==========
        console.error(`[DEBUG] [VIEW TEST] Failed to create resource: ${resourceName}`, error);
        throw error;
        // ========== DEBUG: END ==========
    }
    ...
    try {
        ...
        console.log(`[DEBUG] [VIEW TEST] Resource verified in K8s API: ${resourceName}`);
    } catch (verifyError) {
        console.error(`[DEBUG] [VIEW TEST] CRITICAL: Resource NOT found in K8s API: ${resourceName}`, verifyError);
    }
    // ========== DEBUG: END ==========
