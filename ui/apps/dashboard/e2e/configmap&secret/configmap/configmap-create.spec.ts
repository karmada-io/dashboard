...
import { setupDashboardAuthentication, generateTestConfigMapYaml, deleteK8sConfigMap } from './test-utils';
...
test('should create a new configmap', async ({ page }) => {
    ...
    const apiRequestPromise = page.waitForResponse(response => {
        return response.url().includes('/api/v1/_raw/ConfigMap') && response.status() === 200;
    }, { timeout: 15000 });
    ...
    // Verify new configmap appears in list
    const yamlObject = parse(testConfigMapYaml);
    const configMapName = _.get(yamlObject,'metadata.name');

    // Assert configmap name exists
    expect(configMapName).toBeTruthy();
    expect(configMapName).toBeDefined();

    try {
        await expect(page.locator('table').locator(`text=${configMapName}`)).toBeVisible({
            timeout: 15000
        });
    } catch {
        // If not shown immediately in list, may be due to cache or refresh delay
        // But API success indicates configmap was created
    }
    ...
    if(process.env.DEBUG === 'true'){
        await page.screenshot({ path: 'debug-configmap-create.png', fullPage: true });
    }
