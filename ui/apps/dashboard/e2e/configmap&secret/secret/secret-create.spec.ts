...
import { setupDashboardAuthentication, generateTestSecretYaml, deleteK8sSecret } from './test-utils';
...
test('should create a new secret', async ({ page }) => {
    ...
    const apiRequestPromise = page.waitForResponse(response => {
        return response.url().includes('/api/v1/_raw/Secret') && response.status() === 200;
    }, { timeout: 15000 });
    ...
    // Verify new secret appears in list
    const yamlObject = parse(testSecretYaml);
    const secretName = _.get(yamlObject,'metadata.name');

    // Assert secret name exists
    expect(secretName).toBeTruthy();
    expect(secretName).toBeDefined();

    try {
        await expect(page.locator('table').locator(`text=${secretName}`)).toBeVisible({
            timeout: 15000
        });
    } catch {
        // If not shown immediately in list, may be due to cache or refresh delay
        // But API success indicates secret was created
    }
    ...
    if(process.env.DEBUG === 'true'){
        await page.screenshot({ path: 'debug-secret-create.png', fullPage: true });
    }
