...
import { setupDashboardAuthentication, generateTestIngressYaml, deleteK8sIngress } from './test-utils';
...
test('should create a new ingress', async ({ page }) => {
    ...
    const apiRequestPromise = page.waitForResponse(response => {
        return response.url().includes('/api/v1/_raw/Ingress') && response.status() === 200;
    }, { timeout: 15000 });
    ...
    // Verify new ingress appears in list
    const yamlObject = parse(testIngressYaml);
    const ingressName = _.get(yamlObject,'metadata.name');

    // Assert ingress name exists
    expect(ingressName).toBeTruthy();
    expect(ingressName).toBeDefined();

    try {
        await expect(page.locator('table').locator(`text=${ingressName}`)).toBeVisible({
            timeout: 15000
        });
    } catch {
        // If not shown immediately in list, may be due to cache or refresh delay
        // But API success indicates ingress was created
    }
    ...
    if(process.env.DEBUG === 'true'){
        await page.screenshot({ path: 'debug-ingress-create.png', fullPage: true });
    }
