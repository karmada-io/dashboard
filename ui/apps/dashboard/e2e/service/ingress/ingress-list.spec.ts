...
// apps/dashboard/e2e/ingress-list.spec.ts
...
test('should display ingress list', async ({ page }) => {
    // Open Services menu
    ...
    // Verify Ingress list table is visible
    const table = page.locator('table');
    await expect(table).toBeVisible({ timeout: 30000 });

    // Debug
    if (process.env.DEBUG === 'true') {
        await page.screenshot({ path: 'debug-ingress-list.png', fullPage: true });
    }
});
