...
import { setupDashboardAuthentication, generateTestStatefulSetYaml, deleteK8sStatefulSet } from './test-utils';
...
test('should create a new statefulset', async ({ page }) => {
    ...
    const apiRequestPromise = page.waitForResponse(response => {
        return response.url().includes('/api/v1/_raw/StatefulSet') && response.status() === 200;
    }, { timeout: 15000 });
    ...
    // Verify new statefulset appears in list
    const yamlObject = parse(testStatefulSetYaml);
    const statefulSetName = _.get(yamlObject,'metadata.name');

    // Assert statefulset name exists
    expect(statefulSetName).toBeTruthy();
    expect(statefulSetName).toBeDefined();

    try {
        await expect(page.locator('table').locator(`text=${statefulSetName}`)).toBeVisible({
            timeout: 15000
        });
    } catch {
        // If not shown immediately in list, may be due to cache or refresh delay
        // But API success indicates statefulset was created
    }
    ...
    if(process.env.DEBUG === 'true'){
        await page.screenshot({ path: 'debug-statefulset-create.png', fullPage: true });
    }
