...
import { setupDashboardAuthentication, generateTestJobYaml, createK8sJob, getJobNameFromYaml} from './test-utils';

test.beforeEach(async ({ page }) => {
    await setupDashboardAuthentication(page);
});

test('should delete job successfully', async ({ page }) => {
    // Create a test job directly via kubectl to set up test data
    ...
    const deleteApiPromise = page.waitForResponse(response => {
        return response.url().includes('/_raw/job') &&
            response.url().includes(`name/${jobName}`) &&
    ...
    // Verify job no longer exists in table
    await expect(table.locator(`text=${jobName}`)).toHaveCount(0, { timeout: 30000 });

    // Debug
    if(process.env.DEBUG === 'true'){
        await page.screenshot({ path: 'debug-job-delete-kubectl.png', fullPage: true });
    }
});
