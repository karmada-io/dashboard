...
spec:
    ...
    spec:
      containers:
        - name: ${data.spec.template.spec.containers[0].name}
          ...
          command: ${JSON.stringify(data.spec.template.spec.containers[0].command)}
      restartPolicy: ${data.spec.template.spec.restartPolicy}
  backoffLimit: ${data.spec.backoffLimit}`;
    ...
    if(process.env.DEBUG === 'true'){
        await page.screenshot({ path: 'debug-job-edit.png', fullPage: true });
    }
