...
import { setupDashboardAuthentication, generateTestJobYaml, deleteK8sJob } from './test-utils';
...
test('should create a new job', async ({ page }) => {
    ...
    const apiRequestPromise = page.waitForResponse(response => {
        return response.url().includes('/api/v1/_raw/Job') && response.status() === 200;
    }, { timeout: 15000 });
    ...
    if(process.env.DEBUG === 'true'){
        await page.screenshot({ path: 'debug-job-create.png', fullPage: true });
    }
